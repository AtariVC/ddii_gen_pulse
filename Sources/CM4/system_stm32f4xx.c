...
/******************************************************************************/
...
#if defined(MCO1PRE)
  RCC->CFGR |= (MCO1PRE << 24) | (3 << 21);  //clock from PLL
#endif
#if defined(MCO2PRE)
  RCC->CFGR |= (MCO1PRE << 27) | (3 << 30);  //clock from PLL
#endif
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* Configure Flash prefetch, Instruction cache, Data cache and wait state */
//    FLASH->ACR = FLASH_ACR_PRFTEN | FLASH_ACR_ICEN |FLASH_ACR_DCEN |FLASH_ACR_LATENCY_5WS;
    FLASH->ACR = FLASH_ACR_PRFTEN | FLASH_ACR_ICEN |FLASH_ACR_DCEN |FLASH_ACR_LATENCY_NWS;
